...
// Function to ensure the DOM is ready before mounting the UI
function mountUI() {
  // If body doesn't exist yet, wait for it to be created
  if (!document.body) {
    window.addEventListener("DOMContentLoaded", mountUI);
    return;
  }

  // Check if the extension root already exists (might be added by another script)
  const existingRoot = document.getElementById("chrome-extension-filliny");
  if (existingRoot) {
    // Use the existing root
    const root = existingRoot;

    // Check if it already has a shadow root
    ...
    rootIntoShadow.id = "filliny-shadow-root-ui";
