...
const ensureSpinnerAnimation = (): void => {
  // Check if the animation style already exists to avoid duplicates
  if (document.getElementById('filliny-spinner-animation')) return;

  const animation = document.createElement('style');
  animation.id = 'filliny-spinner-animation';
  animation.setAttribute('data-filliny-element', 'true');
  animation.innerHTML = `
