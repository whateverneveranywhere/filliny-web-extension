...
function RHFShadcnDatePicker({
  ...
  const UTCtoUserDate = useMemo(
    () => (externalValue ? safeParseDate(externalValue as string) : undefined),
    [externalValue],
  );
          ...
          <Popover>
            ...
            <PopoverContent className="filliny-w-auto filliny-p-0" align="start">
              <Calendar
                ...
                selected={field?.value ? safeParseDate(field?.value) : UTCtoUserDate}
                disabled={date => date > new Date() || date < new Date('1900-01-01T00:00:00Z')}
                initialFocus
              />
            </PopoverContent>
          </Popover>
